{% macro snowflake__compare_which_query_columns_differ(a_query, b_query, primary_key_columns, columns, event_time) %}
    ...
    calculated as (
        ...
        select
            {% for column in columns %}
                ...
                {# Snowflake fix:
                   audit_helper default uses adapter.quote(column), which creates a."order_item_sk".
                   For normal unquoted Snowflake columns, use unquoted references instead.
                #}
        ...
        from a
        inner join b
            on a.dbt_audit_surrogate_key = b.dbt_audit_surrogate_key
